...
namespace Calculator.Application.Services.Implementations
{
    public class CalculatorOperationManager : ICalculatorOperationManager
    {
        ...
        {
            var operation = this.serviceResolver.ResolveNamed<ICalculatorOperation>(data.Operation);
            var resultResolver = this.serviceResolver.ResolveNamed<ICalculatorOperationResultResolver>(data.ResponseType ?? "COMMON");
